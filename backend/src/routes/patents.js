...
router.get('/pending-count', auth, async (req, res) => {
  try {
    ...
    // Count pending patents for faculty in the incharge's department
    const pendingCount = await Patent.countDocuments({
    ...
    console.error('Error fetching pending patents count:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
