...
router.get('/patent-counts', auth, async (req, res) => {
  try {
    // Only incharge users can access this endpoint
    if (req.user.role !== 'incharge') {
      return res.status(403).json({ error: 'Access denied. Only incharge users can view patent counts.' });
    }
    ...
    const [totalCount, acceptedCount, pendingCount, rejectedCount] = await Promise.all([
      Patent.countDocuments(patentQuery),
      Patent.countDocuments({ ...patentQuery, status: 'Accepted' }),
      Patent.countDocuments({ ...patentQuery, status: 'Pending' }),
      Patent.countDocuments({ ...patentQuery, status: 'Rejected' })
    ]);
    ...
    console.error('Error fetching patent counts:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
