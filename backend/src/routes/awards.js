...
router.get('/award-counts', auth, async (req, res) => {
  try {
    // Only incharge users can access this endpoint
    if (req.user.role !== 'incharge') {
      return res.status(403).json({ error: 'Access denied. Only incharge users can view award counts.' });
    }
    ...
    const [totalCount, acceptedCount, pendingCount, rejectedCount] = await Promise.all([
      Award.countDocuments(awardQuery),
      Award.countDocuments({ ...awardQuery, status: 'Accepted' }),
      Award.countDocuments({ ...awardQuery, status: 'Pending' }),
      Award.countDocuments({ ...awardQuery, status: 'Rejected' })
    ]);
