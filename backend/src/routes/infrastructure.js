...
router.get('/pending-count', auth, async (req, res) => {
  try {
    ...
    // Count pending infrastructures for faculty in the incharge's department
    const pendingCount = await Infrastructure.countDocuments({
    ...
    console.error('Error fetching pending infrastructures count:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
