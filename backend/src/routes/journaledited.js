...
router.get('/journaledited-counts', auth, async (req, res) => {
  try {
    // Only incharge users can access this endpoint
    if (req.user.role !== 'incharge') {
      return res.status(403).json({ error: 'Access denied. Only incharge users can view journaledited counts.' });
    }
    ...
    // Build base journaledited query
    const journaleditedQuery = {
      empId: { $in: facultyEmpIds }
    };

    // Count journalediteds by status
    const [totalCount, acceptedCount, pendingCount, rejectedCount] = await Promise.all([
      JournalEdited.countDocuments(journaleditedQuery),
      JournalEdited.countDocuments({ ...journaleditedQuery, status: 'Accepted' }),
      JournalEdited.countDocuments({ ...journaleditedQuery, status: 'Pending' }),
      JournalEdited.countDocuments({ ...journaleditedQuery, status: 'Rejected' })
    ]);
    ...
    console.error('Error fetching journaledited counts:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
