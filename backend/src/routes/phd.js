...
router.get('/pending-count', auth, async (req, res) => {
  try {
    ...
    // Count pending phds for faculty in the incharge's department
    const pendingCount = await Phd.countDocuments({
    ...
    console.error('Error fetching pending phds count:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
