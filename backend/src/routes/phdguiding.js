...
router.get('/phdguiding-counts', auth, async (req, res) => {
  try {
    // Only incharge users can access this endpoint
    if (req.user.role !== 'incharge') {
      return res.status(403).json({ error: 'Access denied. Only incharge users can view phdguiding counts.' });
    }
    ...
    // Build base phdguiding query
    const phdguidingQuery = {
      empId: { $in: facultyEmpIds }
    };

    // Count phdguidings by status
    const [totalCount, acceptedCount, pendingCount, rejectedCount] = await Promise.all([
      PhdGuiding.countDocuments(phdguidingQuery),
      PhdGuiding.countDocuments({ ...phdguidingQuery, status: 'Accepted' }),
      PhdGuiding.countDocuments({ ...phdguidingQuery, status: 'Pending' }),
      PhdGuiding.countDocuments({ ...phdguidingQuery, status: 'Rejected' })
    ]);
    ...
    console.error('Error fetching phdguiding counts:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
