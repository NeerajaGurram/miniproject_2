...
router.get('/pending-count', auth, async (req, res) => {
  try {
    ...
    // Count pending phd guidings for faculty in the incharge's department
    const pendingCount = await PhdGuiding.countDocuments({
    ...
    console.error('Error fetching pending phd guidings count:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
