...
import { GraduationCap, Upload, Calendar,Check, Ban, Building, User, Award, FileIcon, X, BookOpen } from 'lucide-react';
...
export default function PhDPage() {
  ...
  const [phdsData, setPhdsData] = useState([]);
  const [loadingPhds, setLoadingPhds] = useState(false);
  const [showConfirmDialog, setShowConfirmDialog] = useState(false);
  const [selectedPhd, setSelectedPhd] = useState(null);
  const [actionType, setActionType] = useState('');

  // Fetch PhDs data for incharge
  useEffect(() => {
  ...
  const fetchPhDsData = async () => {
    try {
      setLoadingPhds(true);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/phd?status=Pending`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch phds data');
      }
      ...
      toast.error('Failed to load PhDs data');
    } finally {
      setLoadingPhds(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!formData.guidec.trim()) newErrors.guidec = "Guide college is required";
    if (!formData.guided.trim()) newErrors.guided = "Guide department is required";
    if (!formData.type1) newErrors.type1 = "Status of PhD is required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submissionData.append('guidec', formData.guidec);
      submissionData.append('guided', formData.guided);
      submissionData.append('type1', formData.type1);
      submissionData.append('date1', formData.date1);
      submissionData.append('file', formData.file);

      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/phd`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to submit PhD details');
      }
      ...
      toast.success('PhD details submitted successfully!');
  ...
  const openConfirmDialog = (phd, action) => {
    setSelectedPhd(phd);
  ...
  const handleStatusChange = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to update phd status');
      }

      toast.success(`PhD ${actionType === 'accept' ? 'accepted' : 'rejected'} successfully`);
      fetchPhDsData(); // Refresh the data
    } catch (error) {
      console.error('Error updating phd status:', error);
      toast.error('Failed to update phd status');
    } finally {
      closeConfirmDialog();
    }
  };
      ...
      >
        ...
        <div className="bg-gradient-brand p-6 text-center">
          <div className="flex items-center justify-center mb-2">
            ...
            <h2 className="text-2xl font-bold text-white">PhD Awarded/Submitted/Registered</h2>
          </div>
          <p className="text-brand-cream text-sm">Submit your PhD details</p>
        </div>
        ...
        <div className="space-y-6 p-6">
          ...
          <div>
            <label className="block text-sm font-medium text-brand-primary mb-2">
              Guide College <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <Building className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-brand-secondary" />
              <input
                type="text"
                name="guidec"
                placeholder="Enter guide college"
                required
                value={formData.guidec}
                onChange={handleChange}
                className="w-full p-3 pl-10 border border-brand-cream rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-secondary focus:border-brand-secondary transition-colors text-gray-900 placeholder-gray-500"
              />
            </div>
            {errors.guidec && <p className="text-red-500 text-sm mt-1">{errors.guidec}</p>}
          </div>

          {/* Guide Dept */}
          <div>
            <label className="block text-sm font-medium text-brand-primary mb-2">
              Guide Department <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              <Building className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-brand-secondary" />
              <input
                type="text"
                name="guided"
                placeholder="Enter guide department"
                required
                value={formData.guided}
                onChange={handleChange}
                className="w-full p-3 pl-10 border border-brand-cream rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-secondary focus:border-brand-secondary transition-colors text-gray-900 placeholder-gray-500"
              />
            </div>
            {errors.guided && <p className="text-red-500 text-sm mt-1">{errors.guided}</p>}
          </div>

          {/* Status of PhD */}
          <div>
            <label className="block text-sm font-medium text-brand-primary mb-2">
              PhD Status <span className="text-red-500">*</span>
            </label>
            <div className="relative">
          ...
          <div>
            <label className="block text-sm font-medium text-brand-primary mb-2">
              Date of Award/Submission/Registration <span className="text-red-500">*</span>
            </label>
            <div className="relative">
      ...
      <div className="p-4 max-w-6xl mx-auto">
        <div className="bg-gradient-brand p-6 text-center mb-6 rounded-lg">
          <div className="flex items-center justify-center mb-2">
            ...
            <h2 className="text-2xl font-bold text-white">PhDs Approval</h2>
          </div>
          <p className="text-brand-cream text-sm">Review and approve PhDs from your department</p>
        </div>

        {loadingPhds ? (
          <div className="text-center p-8">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-brand-secondary mx-auto mb-4"></div>
            <p>Loading PhDs data...</p>
          </div>
        ) : phdsData.length > 0 ? (
          <>
            <div className="mb-4">
              <p className="text-brand-primary">
                {phdsData.length} PhD{phdsData.length !== 1 ? 's' : ''} pending approval
              </p>
            </div>
                ...
                <tbody>
                  {phdsData.map((phd, index) => (
                    <tr key={index} className="hover:bg-gray-50 even:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-sm text-gray-700 border-b border-brand-cream text-center">
                        {phd.sdate ? new Date(phd.sdate).toLocaleDateString() : '-'}
                      </td>
                      <td className="py-3 px-4 text-sm text-center border-b border-brand-cream">
                        {phd.path ? (
                          <a
                            href={`${process.env.NEXT_PUBLIC_API_URL}/phd/file/${phd.path}`}
                            target="_blank"
          ...
          <div className="text-center p-8 bg-gradient-subtle rounded-lg border border-brand-cream">
            <p className="text-brand-primary font-medium">No pending PhDs applications found for your department.</p>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg max-w-md w-full mx-4">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to {actionType} this PhD application? This action cannot be undone.
              </p>
