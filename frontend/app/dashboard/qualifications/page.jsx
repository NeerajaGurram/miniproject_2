...
export default function QualificationsPage() {
  ...
  const fetchQualificationsData = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/qualifications?status=Pending`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch qualifications data');
      }
      
      const result = await response.json();
      setQualificationsData(result);
    } catch (error) {
  ...
  const handleStatusChange = async () => {
    try {
      ...
      toast.success(`Qualification ${actionType === 'accept' ? 'accepted' : 'rejected'} successfully`);
      fetchQualificationsData(); // Refresh the data
    } catch (error) {
      ...
      <div className="p-4 max-w-6xl mx-auto">
        <div className="bg-gradient-brand p-6 text-center mb-6 rounded-lg">
          <div className="flex items-center justify-center mb-2">
            ...
            <h2 className="text-2xl font-bold text-white">Qualifications Approval</h2>
          </div>
          <p className="text-brand-cream text-sm">Review and approve qualifications from your department</p>
        </div>
        ...
        {loadingQualifications ? (
          ...
          <>
            <div className="mb-4">
              <p className="text-brand-primary">
                {qualificationsData.length} qualification{qualificationsData.length !== 1 ? 's' : ''} pending approval
              </p>
            </div>
                ...
                <tbody>
                  {qualificationsData.map((qualification, index) => (
                    <tr key={index} className="hover:bg-gray-50 even:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-sm text-gray-700 border-b border-brand-cream text-center">
                        {qualification.date1 ? new Date(qualification.date1).toLocaleDateString() : '-'}
                      </td>
                      <td className="py-3 px-4 text-sm text-center border-b border-brand-cream">
                        {qualification.path ? (
                          <a
                            href={`${process.env.NEXT_PUBLIC_API_URL}/qualifications/file/${qualification.path}`}
                            target="_blank"
                      ...
                      <td className="py-3 px-4 text-sm text-center border-b border-brand-cream">
                        <div className="flex justify-center space-x-2">
                          <button
                            onClick={() => openConfirmDialog(qualification, 'accept')}
                          ...
                          <button
                            onClick={() => openConfirmDialog(qualification, 'reject')}
          ...
          <div className="text-center p-8 bg-gradient-subtle rounded-lg border border-brand-cream">
            <p className="text-brand-primary font-medium">No pending qualifications found for your department.</p>
          </div>
        )}
